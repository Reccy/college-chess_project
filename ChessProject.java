...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
    ...
    */
    public static void main(String[] args) {
        ...
        int selectedMode = JOptionPane.showOptionDialog(chessProject, "Lets play some Chess, choose your AI opponent", "Introduction to AI Continuous Assessment", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[2]);
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        */
        ...
        if (whiteToMove && pieceName.contains("White")) {
            possible = true;
                ...
                */
                ...
                if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
                  ...
                  */
                    ...
                    else if (xMovement != 0 && yMovement == 0 || xMovement == 0) {
                /*
    ...
    private void makeAIMove() {
        ...
        */
        ...
        while (!whitePieces.empty()) {
            Square pieceSquare = (Square) whitePieces.pop();
            String pieceName = pieceSquare.getName();
            Stack pieceMoves = new Stack();
            /*
              We need to identify all the possible moves that can be made by the AI Opponent
            */
            if (pieceName.contains("Knight")) {
                pieceMoves = getKnightMoves(pieceSquare.getPosX(), pieceSquare.getPosY(), pieceSquare.getName());
                ...
                pieceMoves = getQueenMoves(pieceSquare.getPosX(), pieceSquare.getPosY(), pieceSquare.getName());
            } else if (pieceName.contains("King")) {
                pieceMoves = getKingMoves(pieceSquare.getPosX(), pieceSquare.getPosY(), pieceSquare.getName());
            }

            while (!pieceMoves.empty()) {
                tempMove = (Move) pieceMoves.pop();
                completeMoves.push(tempMove);
            }
        }

        Stack<Move> coloredMoves = (Stack<Move>) completeMoves.clone();
        getLandingSquares(coloredMoves);
        printStack(coloredMoves);
        ...
        */
        ...
        if (completeMoves.size() == 0) {
            ...
            */
            ...
            System.out.println("=============================================================");
            Stack<Move> printingMoves = (Stack<Move>) completeMoves.clone();

            while (!printingMoves.empty()) {
                Move testingMove = printingMoves.pop();
                Square square1 = testingMove.getStart();
                Square square2 = testingMove.getLanding();
                System.out.println("The " + square1.getName() + " can move from (" + square1.getPosX() + ", " + square1.getPosY() + ") to the following square: (" + square2.getPosX() + ", " + square2.getPosY() + ")");
            }
            ...
            int startX1 = (startingPoint.getPosX() * 75) + 20;
            int startY1 = (startingPoint.getPosY() * 75) + 20;
            int landingX1 = (landingPoint.getPosX() * 75) + 20;
            int landingY1 = (landingPoint.getPosY() * 75) + 20;
            System.out.println("-------- Move " + startingPoint.getName() + " (" + startingPoint.getPosX() + ", " + startingPoint.getPosY() + ") to (" + landingPoint.getPosX() + ", " + landingPoint.getPosY() + ")");

            Component component = chessBoard.findComponentAt(startX1, startY1);
            Container parent = component.getParent();
            parent.remove(component);
            int panelID = (startingPoint.getPosY() * 8) + startingPoint.getPosX();
            panels = (JPanel) chessBoard.getComponent(panelID);
            panels.setBorder(redBorder);
            parent.validate();

            Component label = chessBoard.findComponentAt(landingX1, landingY1);
            if (label instanceof JLabel) {
                Container parentLanding = label.getParent();
                JLabel awaitingName = (JLabel) label;
                String agentCaptured = awaitingName.getIcon().toString();
                if (agentCaptured.contains("King")) {
                    agentWins = true;
                }
                parentLanding.remove(label);
                parentLanding.validate();
                pieces = new JLabel(new ImageIcon(startingPoint.getName() + ".png"));
                int landingPanelID = (landingPoint.getPosY() * 8) + landingPoint.getPosX();
     ...
     * king in an adjacent square to were the king lands. Additionally, the King can only move one square at a time.
     * <p>
    ...
    Stack<Move> getKingMoves(int x, int y, String piece) {
        ...
        /*
          If we consider the grid above, we can create three different columns to check.
            - if x increases by one square, using the variable tempX1 (x+1)
            - if x decreases by one square, using the variable tempX2 (x-1)
        ...
        */
        ...
        if (!(tempX1 > 7)) {
            ...
            a condition here to check that the new value of x (tempX1) is not greater than 7.
            ...
            */
            ...
            Square tempSquare1 = new Square(tempX1, y, piece);
            Square tempSquare2 = new Square(tempX1, tempY1, piece);
            Square tempSquare3 = new Square(tempX1, tempY2, piece);
            if (checkSurroundingSquares(tempSquare1)) {
                validMove1 = new Move(startingSquare, tempSquare1);
                if (!piecePresent(((tempSquare1.getPosX() * 75) + 20), (((tempSquare1.getPosY() * 75) + 20)))) {
                    moves.push(validMove1);
                } else {
                    if (checkWhiteOpponent(((tempSquare1.getPosX() * 75) + 20), (((tempSquare1.getPosY() * 75) + 20)))) {
                        moves.push(validMove1);
                    }
                }
            }
            if (!(tempY1 > 7)) {
                if (checkSurroundingSquares(tempSquare2)) {
                    validMove2 = new Move(startingSquare, tempSquare2);
                    if (!piecePresent(((tempSquare2.getPosX() * 75) + 20), (((tempSquare2.getPosY() * 75) + 20)))) {
                        moves.push(validMove2);
                    } else {
                        if (checkWhiteOpponent(((tempSquare2.getPosX() * 75) + 20), (((tempSquare2.getPosY() * 75) + 20)))) {
                            moves.push(validMove2);
                        }
                    }
                }
            ...
            if (!(tempY2 < 0)) {
                if (checkSurroundingSquares(tempSquare3)) {
                    validMove3 = new Move(startingSquare, tempSquare3);
                    if (!piecePresent(((tempSquare3.getPosX() * 75) + 20), (((tempSquare3.getPosY() * 75) + 20)))) {
                        moves.push(validMove3);
                    } else {
                        System.out.println("The values that we are going to be looking at are : " + ((tempSquare3.getPosX() * 75) + 20) + " and the y value is : " + ((tempSquare3.getPosY() * 75) + 20));
                        if (checkWhiteOpponent(((tempSquare3.getPosX() * 75) + 20), (((tempSquare3.getPosY() * 75) + 20)))) {
                            moves.push(validMove3);
                        }
                    }
                }
        ...
        if (!(tempX2 < 0)) {
            Square tmp3 = new Square(tempX2, y, piece);
            Square tmp4 = new Square(tempX2, tempY1, piece);
            Square tmp5 = new Square(tempX2, tempY2, piece);
            if (checkSurroundingSquares(tmp3)) {
                validMove1 = new Move(startingSquare, tmp3);
            ...
            if (!(tempY1 > 7)) {
                if (checkSurroundingSquares(tmp4)) {
                    validMove2 = new Move(startingSquare, tmp4);
            ...
            if (!(tempY2 < 0)) {
                if (checkSurroundingSquares(tmp5)) {
                    validMove3 = new Move(startingSquare, tmp5);
                    if (!piecePresent(((tmp5.getPosX() * 75) + 20), (((tmp5.getPosY() * 75) + 20)))) {
                        moves.push(validMove3);
                    } else {
                        if (checkWhiteOpponent(((tmp5.getPosX() * 75) + 20), (((tmp5.getPosY() * 75) + 20)))) {
                            moves.push(validMove3);
                        }
                    }
                }
        ...
        Square tmp7 = new Square(x, tempY1, piece);
        Square tmp8 = new Square(x, tempY2, piece);
        if (!(tempY1 > 7)) {
            if (checkSurroundingSquares(tmp7)) {
                validMove2 = new Move(startingSquare, tmp7);
                if (!piecePresent(((tmp7.getPosX() * 75) + 20), (((tmp7.getPosY() * 75) + 20)))) {
                    moves.push(validMove2);
                } else {
                    if (checkWhiteOpponent(((tmp7.getPosX() * 75) + 20), (((tmp7.getPosY() * 75) + 20)))) {
                        moves.push(validMove2);
                    }
                }
            }
        ...
        if (!(tempY2 < 0)) {
            if (checkSurroundingSquares(tmp8)) {
                validMove3 = new Move(startingSquare, tmp8);
                if (!piecePresent(((tmp8.getPosX() * 75) + 20), (((tmp8.getPosY() * 75) + 20)))) {
                    moves.push(validMove3);
                } else {
                    if (checkWhiteOpponent(((tmp8.getPosX() * 75) + 20), (((tmp8.getPosY() * 75) + 20)))) {
                        moves.push(validMove3);
                    }
                }
            }
  ...
  */
    ...
    Stack<Move> getBishopMoves(int x, int y, String piece) {
        ...
        */
        ...
        for (int i = 1; i < 8; i++) {
            int tempX1 = x + i;
            int tempY1 = y + i;
            if (!(tempX1 > 7 || tempX1 < 0 || tempY1 > 7 || tempY1 < 0)) {
                Square tmp = new Square(tempX1, tempY1, piece);
                validM = new Move(startingSquare, tmp);
                if (!piecePresent(((tmp.getPosX() * 75) + 20), (((tmp.getPosY() * 75) + 20)))) {
                    moves.push(validM);
        ...
        for (int k = 1; k < 8; k++) {
            int tempX2 = x + k;
            int tempY2 = y - k;
            if (!(tempX2 > 7 || tempX2 < 0 || tempY2 > 7 || tempY2 < 0)) {
                Square tmpK1 = new Square(tempX2, tempY2, piece);
                validM2 = new Move(startingSquare, tmpK1);
                if (!piecePresent(((tmpK1.getPosX() * 75) + 20), (((tmpK1.getPosY() * 75) + 20)))) {
                    moves.push(validM2);
                } else {
                    if (checkWhiteOpponent(((tmpK1.getPosX() * 75) + 20), ((tmpK1.getPosY() * 75) + 20))) {
                        moves.push(validM2);
                        break;
                    } else {
                        break;
        ...
        for (int l = 1; l < 8; l++) {
            int tempX3 = x - l;
            int tempY3 = y + l;
            if (!(tempX3 > 7 || tempX3 < 0 || tempY3 > 7 || tempY3 < 0)) {
                Square tmpLMov2 = new Square(tempX3, tempY3, piece);
                validM3 = new Move(startingSquare, tmpLMov2);
                if (!piecePresent(((tmpLMov2.getPosX() * 75) + 20), (((tmpLMov2.getPosY() * 75) + 20)))) {
                    moves.push(validM3);
                } else {
                    if (checkWhiteOpponent(((tmpLMov2.getPosX() * 75) + 20), ((tmpLMov2.getPosY() * 75) + 20))) {
                        moves.push(validM3);
                        break;
                    } else {
                        break;
        ...
        for (int n = 1; n < 8; n++) {
            int tempX4 = x - n;
            int tempY4 = y - n;
            if (!(tempX4 > 7 || tempX4 < 0 || tempY4 > 7 || tempY4 < 0)) {
                Square tmpNmov2 = new Square(tempX4, tempY4, piece);
                validM4 = new Move(startingSquare, tmpNmov2);
                if (!piecePresent(((tmpNmov2.getPosX() * 75) + 20), (((tmpNmov2.getPosY() * 75) + 20)))) {
                    moves.push(validM4);
                } else {
                    if (checkWhiteOpponent(((tmpNmov2.getPosX() * 75) + 20), ((tmpNmov2.getPosY() * 75) + 20))) {
                        moves.push(validM4);
                        break;
                    } else {
                        break;
    ...
    Stack<Move> getKnightMoves(int x, int y, String piece) {
        ...
        for (int i = 0; i < 8; i++) {
            Square temp = (Square) squares.pop();
            Move tempMove = new Move(startingSquare, temp);
            if ((temp.getPosX() < 0) || (temp.getPosX() > 7) || (temp.getPosY() < 0) || (temp.getPosY() > 7)) {
                // TODO: Find a reason why this empty if statement exists
                // Why is there an empty if statement here????
            } else if (piecePresent(((temp.getPosX() * 75) + 20), (((temp.getPosY() * 75) + 20)))) {
                if (piece.contains("White")) {
                    if (checkWhiteOpponent(((temp.getPosX() * 75) + 20), ((temp.getPosY() * 75) + 20))) {
                        attackingMove.push(tempMove);
                    }
                }
            } else {
                attackingMove.push(tempMove);
    ...
    private void resetBorders() {
        ...
        for (int i = 0; i < 64; i++) {
            JPanel tempPanel = (JPanel) chessBoard.getComponent(i);
            tempPanel.setBorder(empty);
        }
    }
    ...
    private void printStack(Stack input) {
        ...
        while (!input.empty()) {
            ...
            s = move.getStart();
            l = move.getLanding();
            System.out.println("The possible move that was found is : (" + s.getPosX() + " , " + s.getPosY() + "), landing at (" + l.getPosX() + " , " + l.getPosY() + ")");
        }
    }
    ...
    public Boolean piecePresent(Square square) {
        Component c = chessBoard.findComponentAt((square.getPosX() * 75) + 20, (square.getPosY() * 75) + 20);
        return !(c instanceof JPanel);
    }
     ...
     */
    public Boolean checkWhiteOpponent(int newX, int newY) {
     ...
     */
    public Boolean checkBlackOpponent(int newX, int newY) {
        try {
            Component component = chessBoard.findComponentAt(newX, newY);
            JLabel awaitingPiece = (JLabel) component;
            String temp = awaitingPiece.getIcon().toString();
            return (temp.contains("White"));
